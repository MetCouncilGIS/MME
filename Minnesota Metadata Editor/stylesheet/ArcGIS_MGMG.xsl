...
-->
  ...
  <xsl:import href="ArcGIS_Imports\MGMG.xslt"/>
  ...
  <xsl:variable name="mgmg" select="/metadata/idinfo//* | /metadata/metainfo//* | /metadata/dataqual//* | /metadata/spref//* | /metadata/distinfo//*" />
  ...
  <head>
    ...
    <xsl:if test="not (($mgmg[text()]) and (/metadata/metainfo/metstdn = 'Minnesota Geographic Metadata Guidelines'))">
  ...
  <xsl:if test="$flowdirection = 'RTL'">
	...
	<xsl:choose>
		...
		<xsl:when test="($arcgis1[text()] or $arcgis2[text()] or $arcgis3[text()] or $fgdc[text()]) and not (/metadata/metainfo/metstdn = 'Minnesota Geographic Metadata Guidelines')">
			<xsl:call-template name="iteminfo"/> 
		</xsl:when>
	</xsl:choose>

	<xsl:if test="($arcgis1[text()] or $arcgis2[text()] or $arcgis3[text()]) and not (/metadata/metainfo/metstdn = 'Minnesota Geographic Metadata Guidelines')">
  ...
  <xsl:if test="($mgmg[text()]) and (/metadata/metainfo/metstdn = 'Minnesota Geographic Metadata Guidelines')">
      <h2 class="mgmg head"><center>Minnesota Geographic Metadata</center>
      </h2>
      <xsl:call-template name="mgmg"/>
    </xsl:if>  
    
	<xsl:if test="($fgdc[text()]) and not (/metadata/metainfo/metstdn = 'Minnesota Geographic Metadata Guidelines')">
	...
	<xsl:if test="not($arcgis1[text()] or $arcgis2[text()] or $arcgis3[text()] or $fgdc[text()] or $iso19139 or (/metadata/metainfo/metstdn = 'Minnesota Geographic Metadata Guidelines'))">
